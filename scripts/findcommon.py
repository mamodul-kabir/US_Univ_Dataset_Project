...
if common_unitids:
    ...
    if os.path.exists(hd2024_path):
        ...
        df_2024 = pd.read_csv(hd2024_path, usecols=[unitid_col_24, instnm_col_24], encoding='cp1252', encoding_errors='replace')
        df_2024 = df_2024.rename(columns={unitid_col_24: 'UNITID', instnm_col_24: 'INSTNM'})
        
        common_universities_df = df_2024[df_2024['UNITID'].isin(common_unitids)]
        
        os.makedirs("data_clean", exist_ok=True)
        
        output_path = "data_clean/common_universities_2019_2024.csv"
        common_universities_df.to_csv(output_path, index=False)
        print(f"Saved the list of common universities to {output_path}")
